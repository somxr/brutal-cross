...
func update(input: InputPackage, delta: float):
	...
	handle_animation_values(angle,input,delta)
...
func rotate_velocity(input: InputPackage, delta: float):
	...
	if input.input_direction:
		...
		if abs(angle) >= current_angular_speed * delta:
			# Rotate the velocity vector by angular speed each frame. So the turning is gradual. Sign(angle) just decides if clockwise or anticlockwise
	...
	# Final velocity is determined based on the branching of the if statements, whatever it is gets assigned as the final player velocity here		
	player.velocity = current_velocity


func handle_animation_values(angle, input, delta):
	if input.input_direction:
		...
		# #####################################################################
	elif not player.velocity.is_equal_approx(Vector3.ZERO):
		#Gliding is only true if there's no input direction AND player velocity is not zero yet
		is_gliding = true
